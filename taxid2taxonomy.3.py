...
#        
#    Usage: 
#    ./taxid2taxonomy.3.py -i <infile> -n <names.dmp> -d <nodes.dmp>
#    -o <outfile>
#
#    Description:
...
#
#    Date:
#    v1:2014-08-08, v2:2014-08-11, v3:2014-09-19
#
#    Author:
...
if not options.infile or not options.names or not options.nodes or not options.outfile: sys.exit(parser.print_help())
...
header = "identifier\ttaxid\tgroup\t"+"\t".join(wanted_ranks)+"\tlowest_rank_name\tfull_lineage\n"
...
for line in open(options.infile):
    ...
    try:
        ...
        try:
            ...
            print "Warning: Taxid %s for sequence %s is not available locally or online. Skipping sequence." % (taxid, seqid)
            continue
    
    # Go through lineage, adding wanted rank names
    wanted_rank_lineage = dict(zip(wanted_ranks, ['','','','','','','']))
    previous_rank = ''
    ...
    for wanted_rank in wanted_ranks:
        if wanted_rank_lineage[wanted_rank] == '':
            wanted_rank_lineage_names.append('N/A')
        else:
    ...
    output = "\t".join([identifier, taxid, group, "\t".join(wanted_rank_lineage_names), lowest_rank_name, "|".join(full_lineage_names)]) + "\n"
